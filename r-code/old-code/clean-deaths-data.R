...
# Read in data
tb <- read_csv("../COVID-19/csse_covid_19_data/csse_covid_19_time_series/time_series_19-covid-Deaths.csv") %>% 
...
# Tidy and add dates
tb <- tb %>% 
  pivot_longer(cols = -`Province/State`, names_to = "Date", values_to = "Deaths") %>% 
...
out <- left_join(tb, statepop)  %>% 
  mutate(`Deaths per million residents` = 10^6 * Deaths / population)
  
write_csv(out, "shiny/sars-cov-2-usa-by-state/data/us-deaths-cleaned.csv")
write_csv(out, "data/us-deaths-cleaned.csv")
