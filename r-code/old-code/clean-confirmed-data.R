...
# Read in data
tb <- read_csv("../COVID-19/csse_covid_19_data/csse_covid_19_time_series/time_series_19-covid-Confirmed.csv") %>% 
...
# Tidy and add dates
tb <- tb %>% 
  pivot_longer(cols = -`Province/State`, names_to = "Date", values_to = "Confirmed") %>% 
...
write_csv(out, "shiny/sars-cov-2-usa-by-state/data/us-confirmed-cleaned.csv")
write_csv(out, "data/us-confirmed-cleaned.csv")
